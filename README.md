...
| **The fix works** | **implemented, compiled, tested, committed.** `ReconcileModifierCache` at the top of `PrepareInjectedInput`; `test:x86` 19/19, `test:x64` 18/18 at the original landing, both engine DLLs link clean. 64 production lines across 3 files. The ten-commit follow-on took this to 33/33 and 32/32 and closed four of the review's six items. **Since grown further** — a `host32` reproduction-harness round and four commits closing residual pathways (the pass-through race, the eaten-event pipeline loss, and the OSK teardown chirality collapse) — current gates are **`test:x86` 72 pass, `test:x64` 71 pass, 1 disabled each**. `IN-TREE.md` §2, §2a |
| **The pass-through race and the eaten-event pipeline loss** — both left as open items in the review's item 2/3 territory | **fixed and unit-tested, 2026-08-27.** A post-batch verification pass injects a corrective KEYUP when the OS still holds a modifier the cache says is up; the low-level hook now falls through to `CallNextHookEx` instead of eating an event whose handoff to the serializer failed. `IN-TREE.md` §2a |
| **The OSK teardown chirality collapse** (`SetLRShift`) | **fixed, but UNVERIFIED — Delphi unavailable, so nothing compiled or ran.** `ResetShiftStates` now releases by injected chiral identity rather than through the collapsing `ShiftStateChange` path. The **live click-off path is still open** — a fix was written and reverted because the function it would change is also reached from a resync that presses physically-held modifiers, and fixing it risks reintroducing the I2177 regression. `IN-TREE.md` §2a, `REVIEW-8064-reconcile-modifier-cache.md` item 3 |
